...
int testResult() {
	...
	result = result && msg->opcode == OP_SIZE+1 &&
			   msg->c_type == CT_RESULT &&
			   msg->content.result == 15;
...
int testKey() {
	...
	result = result && msg->opcode == OP_DEL &&
			   msg->c_type == CT_KEY &&
...
int testValue() {
	...
	msg->c_type = CT_VALUE;
	datastr = strdup("abc");
	msg->content.value = data_create2(strlen("abc") + 1, datastr);
	size = message_to_buffer(msg, &msg_str);
	...
	result = result && msg->opcode == OP_GET+1 &&
			   msg->c_type == CT_VALUE &&
...
int testEntry() {
	...
	result = result && msg->opcode == OP_PUT &&
			   msg->c_type == CT_ENTRY &&
...
int testKeys() {
	...
	char *msg_str = NULL;
	struct message_t *msg;
	char *keys[] = {"ul2018", "SD", "teste", "123", NULL};
...
int testError() {
	...
	printf("Módulo mensagem -> teste - Error:");
	...
	result = result && msg->opcode == OP_ERROR &&
			   msg->c_type == CT_NONE;
...
int main() {
	...
	printf("Resultados do teste do módulo message: %d em 7\n\n",score);
